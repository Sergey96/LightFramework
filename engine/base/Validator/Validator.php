...
namespace engine\base\Validator;
...
class Validator
{
	public static function validate($type, $value, $params = array()){
		...
		switch($group){
			case 'int': $result = self::validateInt($value, $type); break;
			case 'decimal': $result = self::validateDecimal($value, $type); break;
			case 'float': $result = self::validateFloat($value, $type); break;
			case 'varchar': $result = self::validateChar($value, $type); break;
			case 'text': $result = self::validateText($value, $type); break;
			case 'datetime': $result = self::validateDate($value, $type); break;
		}
		return $result;
	}
	
	public static function validateInt($value, $type){
		if(is_int($value+1-1) && self::$MYSQL_TYPES[$type]['min']<$value && $value<self::$MYSQL_TYPES[$type]['max'])
			return true;
	}
	...
	public static function validateDate($value, $type){
		switch($type){
			...
			case 'date': return self::validateDateTime($value, 'Y-m-d');
			case 'year': return (self::MYSQL_TYPES[$type]['min'] < $value &&  $value < self::MYSQL_TYPES[$type]['max']);
